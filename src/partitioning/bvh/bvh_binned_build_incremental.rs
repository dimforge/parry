use super::bvh_tree::{BvhNodeVec, BvhNodeWide};
...
pub struct BinnedRebuildState {
    partial_tree: BvhNodeVec,
    substeps: VecDeque<RebuildSubstep>,
    leaves_to_sort: Vec<BvhNode>,
}

impl BinnedRebuildState {
    pub fn step_rebuild(&mut self, tree: &mut Bvh) -> bool {
        if self.substeps.is_empty() {
            ...
            self.partial_tree.push(BvhNodeWide::zeros());
        ...
        if ALWAYS_UPDATE_LEAF_STATE {
            for leaf in &mut self.leaves_to_sort {
                let leaf_index = tree.leaf_node_indices[leaf.children as usize];
        ...
        if self.substeps.is_empty() {
            ...
            for node in self.partial_tree.iter_mut() {
                if node.left.is_leaf() {
                    let leaf_index = tree.leaf_node_indices[node.left.children as usize];
                    node.left = tree.nodes[leaf_index];
                }
                if node.right.is_leaf() {
                    let leaf_index = tree.leaf_node_indices[node.right.children as usize];
                    node.right = tree.nodes[leaf_index];
                }
            }
            // 2. Refit.
            Bvh::refit_buffers(
                &mut self.partial_tree,
                &mut tree.nodes,
                &mut tree.leaf_node_indices,
                &mut tree.parents,
            );
