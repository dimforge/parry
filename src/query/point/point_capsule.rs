...
impl PointQuery for Capsule {
    ...
    fn project_local_point(&self, pt: &Point<f32>, solid: bool) -> PointProjection {
        ...
        if let Some((dir, dist)) = Unit::try_new_and_get(dproj, f32::default_epsilon()) {
            ...
            if solid && inside {
                ...
                return PointProjection::new(
                    inside,
                    proj.local_point + dir.into_inner() * self.radius,
                );
            }
        } else if solid {
            return PointProjection::new(true, *pt);
        ...
        if let Some(dir) = seg.normal() {
            PointProjection::new(true, proj.local_point + *dir * self.radius)
        } else {
            // The segment has no normal, likely because it degenerates to a point.
            PointProjection::new(true, proj.local_point + Vector::ith(1, self.radius))
        }
