...
pub fn contact_ball_ball(
    ...
    let r1 = b1.radius;
    let r2 = b2.radius;
    let center2_1 = pos12.translation.into_inner();
    ...
    if distance_squared < sum_radius_with_error * sum_radius_with_error {
        let normal1 = if !distance_squared.is_zero() {
            UnitVector::new_normalize(center2_1)
        } else {
            Vector::x_axis()
        };
        let normal2 = -pos12.inverse_transform_unit_vector(&normal1);
        let point1 = Point::from(normal1.into_inner() * r1);
        let point2 = Point::from(normal2.into_inner() * r2);
