...
impl RayCast for HalfSpace {
    ...
    fn cast_local_ray_and_get_normal(
        ...
        if solid && dot_normal_dpos > real!(0.0) {
            // The ray is inside of the solid half-space.
            return Some(RayIntersection::new(real!(0.0), na::zero(), FeatureId::Face(0)));
        }

        let t = dot_normal_dpos / self.normal.dot(&ray.dir);

        if t >= real!(0.0) && t <= max_toi {
            let n = if dot_normal_dpos > real!(0.0) {
                -self.normal
            } else {
                self.normal
            };
