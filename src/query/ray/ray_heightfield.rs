...
impl RayCast for HeightField {
    ...
    fn cast_local_ray_and_get_normal(
        ...
        if let Some(seg) = self.segment_at(curr) {
            ...
            if s >= na::zero::<Real>() && t >= na::zero::<Real>() && t <= na::one::<Real>() {
                // Cast succeeded on the first element!
                let n = seg.normal().unwrap().into_inner();
                let fid = if n.dot(&ray.dir) > na::zero::<Real>() {
         ...
         */
        if ray.dir.x == na::zero::<Real>() {
            return None;
        }

        let right = ray.dir.x > na::zero::<Real>();
        ...
        while (right && curr < self.num_cells()) || (!right && curr > 0) {
            ...
            if let Some(seg) = self.segment_at(curr) {
                ...
                if t >= na::zero::<Real>() && t <= na::one::<Real>() && s <= max_toi {
                    let n = seg.normal().unwrap().into_inner();
                    let fid = if n.dot(&ray.dir) > na::zero::<Real>() {
                        // The ray hit the back face.
                        curr + self.num_cells()
...
impl RayCast for HeightField {
    ...
    fn cast_local_ray_and_get_normal(
        ...
        let mut cell = match self.cell_at_point(&clip_ray_a) {
            ...
            // None may happen due to slight numerical errors.
            None => {
                let i = if ray.origin.z > na::zero::<Real>() {
                    self.nrows() - 1
                } else {
                    0
                };

                let j = if ray.origin.x > na::zero::<Real>() {
                    self.ncols() - 1
                } else {
                    0
                };
        ...
        loop {
             ...
             */
            let (toi_x, right) = if ray.dir.x > na::zero::<Real>() {
                let x = self.x_at(cell.1 + 1);
                ((x - ray.origin.x) / ray.dir.x, true)
            } else if ray.dir.x < na::zero::<Real>() {
            ...
            let (toi_z, down) = if ray.dir.z > na::zero::<Real>() {
                let z = self.z_at(cell.0 + 1);
                ((z - ray.origin.z) / ray.dir.z, true)
            } else if ray.dir.z < na::zero::<Real>() {
            ...
            if toi_x >= na::zero::<Real>() && toi_x < toi_z {
                if right {
                    cell.1 += 1
                } else if cell.1 > 0 {
                    cell.1 -= 1
