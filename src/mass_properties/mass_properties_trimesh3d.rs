...
pub fn tetrahedron_unit_inertia_tensor_wrt_point(
    ...
    let a0 = (diag_y + diag_z) * real!(0.1);
    let b0 = (diag_z + diag_x) * real!(0.1);
    let c0 = (diag_x + diag_y) * real!(0.1);

    let a1 = (y1 * z1 * real!(2.0)
        + y2 * z1
        + y3 * z1
        + y4 * z1
        + y1 * z2
...
pub fn trimesh_signed_volume_and_center_of_mass(
    ...
    let geometric_center = Point::new(real!(-10.0), real!(-10.0), real!(-10.0)); // utils::center(vertices);

    let mut res = Point::origin();
    let mut vol = real!(0.0);
...
mod test {
        ...
        for k in 0..3 {
            let i1 = mprops1.principal_inertia_local_frame
                * mprops1.principal_inertia().component_mul(
                    &(mprops1.principal_inertia_local_frame.inverse() * Vector::ith(k, real!(1.0))),
                );
            let i2 = mprops2.principal_inertia_local_frame
                * mprops2.principal_inertia().component_mul(
                    &(mprops2.principal_inertia_local_frame.inverse() * Vector::ith(k, real!(1.0))),
                );
            assert_relative_eq!(i1, i2, epsilon = real!(0.5))
        }
    }

    #[test]
    fn cuboid_as_trimesh_mprops() {
        let cuboid = Cuboid::new(Vector::new(real!(1.0), real!(2.0), real!(3.0)));

        use crate::shape::Shape;
        let orig_mprops = cuboid.mass_properties(real!(1.0));
        dbg!(orig_mprops.principal_inertia());

        let mut trimesh = cuboid.to_trimesh();
        let mprops = MassProperties::from_trimesh(real!(1.0), &trimesh.0, &trimesh.1);
        assert_relative_eq!(mprops.mass(), real!(48.0), epsilon = real!(1.0e-4));
        assert_relative_eq!(
            (mprops.principal_inertia_local_frame * mprops.principal_inertia()).abs(),
            Vector::new(real!(208.0), real!(160.0), real!(80.0)),
            epsilon = real!(1.0e-4)
        );
        ...
        trimesh
            .0
            .iter_mut()
            .for_each(|pt| *pt += Vector::new(real!(30.0), real!(20.0), real!(10.0)));
        let mprops = MassProperties::from_trimesh(real!(1.0), &trimesh.0, &trimesh.1);
        assert_relative_eq!(mprops.mass(), real!(48.0), epsilon = real!(1.0e-4));
        assert_relative_eq!(
            (mprops.principal_inertia_local_frame * mprops.principal_inertia()).abs(),
    ...
    fn primitives_as_trimesh_mprops() {
        let primitives = (
            Cuboid::new(Vector::new(real!(1.0), real!(2.0), real!(3.0))),
            Capsule::new_y(real!(2.0), real!(1.0)),
            Cone::new(real!(2.0), real!(1.0)),
            Cylinder::new(real!(2.0), real!(1.0)),
            Ball::new(real!(2.0)),
        );
        let mut meshes = [
        ...
        for (shape, mesh) in shapes.iter().zip(meshes.iter_mut()) {
            let shape_mprops = shape.mass_properties(real!(2.0));
            let mesh_mprops = MassProperties::from_trimesh(real!(2.0), &mesh.0, &mesh.1);
            assert_relative_eq!(shape_mprops.mass(), mesh_mprops.mass(), epsilon = real!(1.0e-1));
            ...
            let shift = Vector::new(real!(33.0), real!(22.0), real!(11.0));
