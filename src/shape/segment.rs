...
impl Segment {
    ...
    pub fn direction(&self) -> Option<UnitVector> {
        UnitVector::try_new(self.scaled_direction(), DEFAULT_EPSILON)
    }
    ...
    pub fn planar_normal(&self, plane_axis: u8) -> Option<UnitVector> {
        UnitVector::try_new(self.scaled_planar_normal(plane_axis), DEFAULT_EPSILON)
    }
    ...
    pub fn point_at(&self, location: &SegmentPointLocation) -> Point {
        match *location {
            ...
            SegmentPointLocation::OnEdge(bcoords) => {
                self.a * bcoords[0] + self.b.as_vector() * bcoords[1]
            }
            _ => panic!(),
        }
    }
        ...
        if let Some(direction) = self.direction() {
            match feature {
                ...
                FeatureId::Edge(_) => {
                    ...
                    normal[iamin] = 1.0;
                    normal -= direction.into_inner() * direction[iamin];
...
impl SupportMap for Segment {
    ...
    fn local_support_point(&self, dir: &Vector) -> Point {
        if self.a.as_vector().dot(*dir) > self.b.as_vector().dot(*dir) {
            self.a
        } else {
            self.b
        }
