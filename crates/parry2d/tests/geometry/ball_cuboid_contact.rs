...
fn test_ball_cuboid_query_contact() {
    let cuboid = Cuboid::new(Vector::new(0.5, 0.5));
    let cuboid_pos = Pose::translation(0.0, 4.0);
    let ball = Ball::new(0.5);
    let ball_pos = Pose::translation(0.0517938, 3.05178815);
...
fn test_false_negative() {
    let contact = query::contact(
        &Pose::translation(1.0, 1.0),
        &Ball::new(1.0),
        &Pose::identity(),
        &Cuboid::new(Vector::new(1.0, 1.0)),
        1.0,
    )
    .unwrap()
    .unwrap();
